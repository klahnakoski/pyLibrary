...
class TestJSON(unittest.TestCase):
    ...
    def test_unicode1(self):
        ...
        test1 = typed_encode(value)
        expected = u'{"comment":{' + quote(STRING_KEY) + u':"Open all links in the current tab, except the pages opened from external apps â€” open these ones in new windows"},' + quote(EXISTS_KEY) + u':1}'
        self.assertEqual(test1, expected)

    def test_unicode2(self):
        value = {"comment": "testing accented char àáâãäåæçèéêëìíîïðñòóôõö÷øùúûüýþÿ"}
        test1 = typed_encode(value)
        expected = u'{"comment":{' + quote(STRING_KEY) + u':"testing accented char àáâãäåæçèéêëìíîïðñòóôõö÷øùúûüýþÿ"},' + quote(EXISTS_KEY) + u':1}'
        self.assertEqual(test1, expected)

    def test_unicode3(self):
        value = {"comment": u"testing accented char ŕáâăäĺćçčéęëěíîďđńňóôőö÷řůúűüýţ˙"}
        test1 = typed_encode(value)
        expected = u'{"comment":{' + quote(STRING_KEY) + u':"testing accented char ŕáâăäĺćçčéęëěíîďđńňóôőö÷řůúűüýţ˙"},' + quote(EXISTS_KEY) + u':1}'
        self.assertEqual(test1, expected)
    ...
    def test_nested(self):
        value = {"a": {}, "b": {}}
        test1 = typed_encode(value)
        expected = u'{"a":{' + quote(EXISTS_KEY) + u':1},"b":{' + quote(EXISTS_KEY) + u':1},' + quote(EXISTS_KEY) + u':1}'
        self.assertEqual(test1, expected)

    def test_list_of_objects(self):
        value = {"a": [{}, "b"]}
        test1 = typed_encode(value)
        expected = u'{"a":{"'+ARRAY_KEY+u'":[{' + quote(EXISTS_KEY) + u':1},{' + quote(STRING_KEY) + u':"b"}]},' + quote(EXISTS_KEY) + u':1}'
        self.assertEqual(test1, expected)
    ...
    def test_list_n(self):
        value = {"value": [23.5, 42]}
        test1 = typed_encode(value)
        expected = u'{"value":[{' + quote(NUMBER_KEY) + u':23.5},{' + quote(NUMBER_KEY) + u':42}],' + quote(EXISTS_KEY) + u':1}'
        self.assertEqual(test1, expected)
    ...
    def test_complex_object(self):
        value = wrap({"s": 0, "r": 5})
        test1 = typed_encode(value)
        expected = u'{"r":{' + quote(NUMBER_KEY) + u':5},"s":{' + quote(NUMBER_KEY) + u':0},' + quote(EXISTS_KEY) + u':1}'
        self.assertEqual(test1, expected)
    ...
    def test_empty_object_in_list(self):
        ...
        expected = u'{"a":{"b":{' + quote(EXISTS_KEY) + u':1},' + quote(EXISTS_KEY) + u':1},' + quote(EXISTS_KEY) + u':1}'
